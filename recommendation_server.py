...
class RecommendationService(demo_pb2_grpc.RecommendationServiceServicer):
    def ListRecommendations(self, request, context):
        logger.info("execute ListRecommendataions")
        max_responses = 5
        # fetch list of products from product catalog stub
        cat_response = product_catalog_stub.ListProducts(demo_pb2.Empty()) # REVIEW: read
...
if __name__ == "__main__":
    ...
    product_catalog_stub = demo_pb2_grpc.ProductCatalogServiceStub(channel) # Review: done

    # create gRPC server
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))

    # add class to gRPC server
    service = RecommendationService() # REVIEW: done
